...
<?php }else{?>
	...
	"action" : function (obj) {
                             ...
                             $.ajax({
                                   type:"POST",
			           url:"<?php echo $baseUrl;?>/examination/returnView",
		                   data:   {
                                             "id" :id,
                                            "YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"
                                              },
...
"remove" : {
		       ...
		       $('<div title="Delete Confirmation">\n\
                    ...
                    Examination <span style="color:#FF73B4;font-weight:bold;">'+(obj).attr('rel')+'</span> and all it\'s subcategories will be deleted.Are you sure?</div>')
                       .dialog({
			resizable: false,
			height:170,
			modal: true,
			buttons: {
				       "Delete": function() {
                                        jQuery("#<?php echo Examination::ADMIN_TREE_CONTAINER_ID;?>").jstree("remove",obj);
					$( this ).dialog( "close" );
				},
				Cancel: function() {
					$( this ).dialog( "close" );
...
<?php if(!Yii::app()->user->isGuest){?>
		...
		$.ajax({
                           type:"POST",
			   url:"<?php echo $baseUrl;?>/examination/rename",
			   data:  {
				        "id" : data.rslt.obj.attr("id").replace("node_",""),
		...
		$.ajax({
                           type:"POST",
			    url:"<?php echo $baseUrl;?>/examination/remove",
			    data:{
				        "id" : data.rslt.obj.attr("id").replace("node_",""),
			                "YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"
                                        },
...
.bind("move_node.jstree", function (e, data) {
		data.rslt.o.each(function (i) {
                ...
                //Commented out logs  are for debugging and exploration of jstree.
                 ...
                 next= jQuery.jstree._reference('#<?php echo Examination::ADMIN_TREE_CONTAINER_ID;?>')._get_next (this, true);
                 previous= jQuery.jstree._reference('#<?php echo Examination::ADMIN_TREE_CONTAINER_ID;?>')._get_prev(this,true);
...
//                      console.log(replaced_node,'REPLACED NODE');
			...
			$.ajax({
				async : false,
				type: 'POST',
				url: "<?php echo $baseUrl;?>/examination/moveCopy",
