...
<div id="<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>" >

</div>

<script  type="text/javascript">
$(function () {
$("#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>")
		.jstree({
                           "html_data" : {
	            "ajax" : {
                                 "type":"POST",
 	                          "url" : "<?php echo $baseUrl;?>/organization/fetchTree",
	                         "data" : function (n) {
	                          return {
...
"update" : {
	              ...
	              "action"	: function (obj) {
                     ...
                     $.ajax({
                                 type: "POST",
                                 url: "<?php echo $baseUrl;?>/organization/returnForm",
                                data:{
                                          'update_id':  id,
                                           "YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"
                                              },
			       'beforeSend' : function(){
                                           $("#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>").addClass("ajax-sending");
                                                             },
                               'complete' : function(){
                                           $("#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>").removeClass("ajax-sending");
                                                   },
	...
	"action" : function (obj) {
                             ...
                             $.ajax({
                                   type:"POST",
			           url:"<?php echo $baseUrl;?>/organization/returnView",
		                   data:   {
                                             "id" :id,
                                            "YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"
                                              },
...
"remove" : {
		       ...
		       $('<div title="Delete Confirmation">\n\
                    ...
                    Organization <span style="color:#FF73B4;font-weight:bold;">'+(obj).attr('rel')+'</span> and all it\'s subcategories will be deleted.Are you sure?</div>')
                       .dialog({
			resizable: false,
			height:170,
			modal: true,
			buttons: {
				       "Delete": function() {
                                        jQuery("#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>").jstree("remove",obj);
					$( this ).dialog( "close" );
				},
				Cancel: function() {
					$( this ).dialog( "close" );
...
//	}//list products
		...
		$.ajax({
                           type:"POST",
			   url:"<?php echo $baseUrl;?>/organization/rename",
			   data:  {
		...
		$.ajax({
                           type:"POST",
			    url:"<?php echo $baseUrl;?>/organization/remove",
			    data:{
				        "id" : data.rslt.obj.attr("id").replace("node_",""),
			                "YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"
                                        },
...
.bind("move_node.jstree", function (e, data) {
		data.rslt.o.each(function (i) {
                ...
                //Commented out logs  are for debugging and exploration of jstree.
                 ...
                 next= jQuery.jstree._reference('#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>')._get_next (this, true);
                 previous= jQuery.jstree._reference('#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>')._get_prev(this,true);
...
//                      console.log(replaced_node,'REPLACED NODE');
			...
			$.ajax({
				async : false,
				type: 'POST',
				url: "<?php echo $baseUrl;?>/organization/moveCopy",
...
//BINDING EVENTS FOR THE ADD ROOT AND REFRESH BUTTONS.
   $("#add_root").click(function () {
	$.ajax({
		...
		jQuery("#<?php echo Organization::ADMIN_TREE_CONTAINER_ID;?>").jstree("refresh");
	});
});
